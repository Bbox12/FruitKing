...
$server_ip="139.59.38.160";
  $response = array("foods"=>array(),"shop"=>array(),"second"=>array(),"bookings"=>array(),"manualbookings"=>array(),"timings"=>array(),"third"=>array(),"qrorders"=>array());


 $manualbookings=mysqli_query($conn, "SELECT (SELECT Name FROM user_details WHERE ID=b.User_ID) AS Name,b.From_Address,b.pCost,b.ID,b.Cost,b.PaymentMode,b.PaymentVerified,b.Is_Paid,b.ETR,(SELECT Name FROM user_details WHERE ID=b.Driver_ID) AS `Driver`,(SELECT Photo FROM user_details WHERE ID=b.Driver_ID) AS `Photo`,b.Vehicle_ID,b.Booking_Date,b.Booking_Time,d.Delivered,b.Date,b.Time FROM `store_order` s INNER JOIN delievered d ON s.OrderID=d.OrderID INNER JOIN book_ride b ON b.OTP=d.OrderID WHERE  s.OrderID='$submenu' AND b.User='$mobile' GROUP BY d.ID");
...
$jsonRow_201=array(
  "ID"=>$user1['ID'],
                     ...
                     "Name"=>$user1['Name'],
                          ...
                          "Booking_Date"=>$user1['Date'],
                          "Booking_Time"=>$user1['Time'],
                         "End_Date"=>$user1['Date'],
                          "End_Time"=>$user1['Time'],
                          ...
                          "Photo"=>'http://' . $server_ip . '/' . 'Fruit'.'/'.'Menu'.'/'.$user1['Photo'],
...
array_push($response["manualbookings"], $jsonRow_201);
 ...
 $qrorders=mysqli_query($conn, "SELECT b.From_Address,b.pCost,b.ID,b.Cost,b.PaymentMode,b.PaymentVerified,b.Is_Paid,b.Amount, b.ChangePaid, b.ETR,(SELECT Name FROM user_details WHERE ID=b.Driver_ID) AS `Driver`,(SELECT Photo FROM user_details WHERE ID=b.Driver_ID) AS `Photo`,b.Vehicle_ID,b.Booking_Date,b.Booking_Time,d.Delivered,b.End_Date,b.End_Time FROM `store_order` s INNER JOIN delievered d ON s.OrderID=d.OrderID INNER JOIN book_ride b ON b.OTP=d.OrderID WHERE  s.OrderID='$submenu' AND b.User='$mobile' GROUP BY d.ID");

while ($user1 = mysqli_fetch_assoc($qrorders)) {
...
$result =$conn->query("SELECT ID FROM  menu_type WHERE Name='$submenu'");
     ...
     $third=mysqli_query($conn, "SELECT f.ID,f.IDMenu,f.IDSubsubmenu,f.isOutOfStock,sm.Category AS Submenu,m.Name AS Menu,m.Colors,f.Name,f.Weight,f.Unit,f.Description,f.MRP,f.JalpanPrice,f.Discount,f.Photo,smsm.Name AS PrimaryCategory FROM `foods` f  INNER JOIN submenu sm ON sm.ID=f.IDSubsubmenu INNER JOIN menu_type m ON m.ID=f.IDMenu INNER JOIN subsubmenu smsm ON smsm.ID=f.IDSubmenu WHERE f.Available=1  AND f.IDMenu='$mID'  ");
