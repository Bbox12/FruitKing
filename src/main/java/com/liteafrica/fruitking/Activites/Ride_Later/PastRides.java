package com.liteafrica.fruitking.Activites.Ride_Later;
...
import com.liteafrica.fruitking.Model.User;
...
public class PastRides extends AppCompatActivity implements View.OnClickListener {
    ...
    private EditText Past_total_Km,_tpaid,_change,_changepaid;
    ...
    private TextView y5;
    private LinearLayout _L1,change;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _tpaid=findViewById(R.id._tpaid);
        _change=findViewById(R.id._change);
        _changepaid=findViewById(R.id._changepaid);
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            if(pref.getResposibility()==3){
                                JSONArray timings = jsonObj.getJSONArray("qrorders");
                                if (timings.length() != 0) {
                                    for (int i = 0; i < timings.length(); i++) {
                                        ...
                                        if (!c.isNull("Delivered")) {
                                            ...
                                            items.setAmount(c.getString("Amount"));
                                            items.setChangePaid(c.getString("ChangePaid"));
                                            mItems.add(items);
                                        }
                                    }
    ...
    private void go(ArrayList<User> mItems) {
        ...
        if (mItems.size() != 0) {
            ...
            if(mItems.get(0).getPaymentMode(0)==3 ){
                String[] pars = mItems.get(0).getAmount(0).split("\\.");
                int _p = Integer.parseInt(pars[0]);
                if(_p>0 ) {
                    change.setVisibility(View.VISIBLE);
                    _tpaid.setText("R" + mItems.get(0).getAmount(0));
                    if (mItems.get(0).getChangePaid(0).equals("1")) {
                        _changepaid.setText("YES");
                    } else {
                        _changepaid.setText("NO");
                    }

                    String[] pars1 = mItems.get(0).getCost(0).split("\\.");
                    int _q = Integer.parseInt(pars1[0]);
                    DecimalFormat df = new DecimalFormat("0.00");
                    if (_p > _q) {
                        int _c = _p - _q;
                        _change.setText("- R" + df.format(_c));
