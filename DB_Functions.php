...
class DB_Functions {
    ...
    function __destruct() {
     ...
     public function add_order_plattersIOS($ID,$mobile,$date,$from,$time,$details,$address){
        ...
        $stmt=$this->conn->prepare("SELECT ID FROM platters WHERE Name=?  ");
 ...
 //printf("pID: %d.\n", $pID);
    ...
    $stmt = $this->conn->prepare("INSERT INTO `book_platters`( `Occasion`,`UserID`, `PlatterID`, `Booking_date`, `Booking_time`, `Description`, `Address`,`Date`,`Time`) VALUES(?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("siissssss",$ID,$mobile,$from,$date,$time,$details,$address,$date,$hour);
        $result = $stmt->execute();
        $error= $stmt->errno;
   //printf("Error: %d.\n", $stmt->errno);
         if( $stmt->errno==0){
     ...
     public function add_order_platters($occ,$mobile,$bookingdate,$from,$bookingtime,$details,$address,$IP){
    ...
    $stmt = $this->conn->prepare("INSERT INTO `book_platters`( `Occasion`,`UserID`, `PlatterID`, `Booking_date`, `Booking_time`, `Description`, `Address`,`Date`,`Time`,`IP`) VALUES(?,?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("siisssssss",$occ,$mobile,$from,$bookingdate,$bookingtime,$details,$address,$date,$hour,$IP);
        $result = $stmt->execute();
        $error= $stmt->errno;
 ...
 public function add_order_Manual($mobile,$pin,$data,$total,$name,$phone,$address,$IP,$pay,$amount,$paid){
              $uID=0;
       ...
       $stmt=$this->conn->prepare("SELECT ID FROM user_details WHERE Phone_No=?  ");
        ...
        $Password=sha1("12345678");
              ...
              if($ParlourID==0){
              $stmt = $this->conn->prepare("INSERT INTO `user_details`(`Name`, `Password`,`Phone_No`, `Date`, `Time`) VALUES(?,?,?,?,?)");
        $stmt->bind_param("sssss", $name,$Password,$phone,$date,$hour);
        $result = $stmt->execute();
        ...
        $stmt->close(); 
         ...
         $stmt = $this->conn->prepare("UPDATE `user_details`SET `Name`=?, `Phone_No`=?,`Password`=?, `Date`=?, `Time`=? WHERE ID=?");
        $stmt->bind_param("sssssi", $name,$phone,$Password,$date,$hour,$ParlourID);
        $result = $stmt->execute();
        $last_id = $ParlourID;
        $error= $stmt->errno;
    ...
    $myArray=array();
        ...
        $dataArray = explode('_', $laterDate);
            ...
            $stmt->bind_param("iiiiiss",$last_id,$otp,$ID,$dataArray[0],$dataArray[1],$date,$hour);
            $result=$stmt->execute();
        ...
        if($result){
           ...
           $stmt=$this->conn->prepare("INSERT INTO `update_user_order`(`UserID`, `OrderID`, `Gross`, `Total`, `Date`, `Time`) VALUES  (?,?,?,?,?,?)");
            $stmt->bind_param("iiddss",$last_id,$otp,$total,$total,$date,$hour);
            $result=$stmt->execute();
             if($stmt->errno==0){
                $result=true;
            }
        ...
        if($result){
                 $stmt=$this->conn->prepare("INSERT INTO `delievered`(`OrderID`,Delivered,`Date`, `Time`) VALUES  (?,5,?,?)");
            ...
            if($error==0){
         $stmt = $this->conn->prepare("INSERT INTO book_ride(`OTP`,`Unique_Ride_Code`,`IDDelivery`, `User_ID`,`From_Address`,`Booking_Date`,`Booking_Time`,Distance_Travel,Cost,`PaymentMode`,`Amount`,`ChangePaid`,Is_Paid,`Date`, `Time`, `User`, `IP`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?)");
        $stmt->bind_param("isiisssddidissss",$otp,$unique_id,$lastid,$last_id,$address,$date,$hour,$distance,$total,$pay,$amount,$paid,$date,$hour,$mobile,$IP);
                $stmt->execute();
          $oId=$stmt->insert_id;
              if($stmt->errno==0){
                $result=true;
 ...
 public function add_order_Fruit($mobile,$ID,$data,$total,$gross,$del,$package,$discount,$address,$from_lat,$from_long,$checked,$IP,$distance,$pay,$charge){
              $uID=0;
...
public function add_later_outstation($mobile,$otp,$vehicle,$cost,$return){
 ...
 if($Ride_ID!=0){
              $stmt = $this->conn->prepare("UPDATE delievered  SET  Delivered=4 WHERE OrderID=?");
 ...
 if($Ride_ID!=0){
   ...
   if($stmt->errno==0){
            ...
            $stmt->close();
              ...
              $stmt = $this->conn->prepare("UPDATE delievered SET DriverID=?,Delivered=6 WHERE OrderID=?  ");
